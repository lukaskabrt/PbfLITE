...
public abstract class PbfReaderPrimitivesTests
{
    public abstract uint ReadFixed32(byte[] data);

    public abstract ulong ReadFixed64(byte[] data);

    public abstract uint ReadVarInt32(byte[] data);

    public abstract ulong ReadVarInt64(byte[] data);

    public abstract ReadOnlySpan<byte> ReadLengthPrefixedBytes(byte[] data);
    ...
    {
        var data = ReadLengthPrefixedBytes([0x03, 0x41, 0x42, 0x43]);
